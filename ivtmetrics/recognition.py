...
class Recognition(Disentangle):
    ...
    def compute_video_all_AP(self, components=["i", "v", "t", "iv", "it", "ivt"], ignore_null=False):
        ...
        @args
        ----
        components: list of str (optional) default: ["i", "v", "t", "iv", "it", "ivt"]
            A list of components of interest. 
            "i" for instruments, "v" for verbs, "t" for targets, "iv" for instrument-verb, 
            "it" for instrument-target, "ivt" for instrument-verb-target.
        @return
        -------
        results: dict
            Dictionary with mean AP and std AP for each component.
        """
        global_targets = self.global_targets
        global_predictions = self.global_predictions

        # Ensure targets and predictions for the last video are added
        if len(self.targets) > 0:
            global_targets.append(self.targets)
            global_predictions.append(self.predictions)

        # Initialize results dictionary to store mean and std for each component
        results = {}
        ...
        with warnings.catch_warnings():
            ...
            # Loop over each specified component to calculate mean and std
            for component in components:
                ...
                for targets, predicts in zip(global_targets, global_predictions):
                    ...
                    # Calculate classwise AP for each video
                    classwise = average_precision_score(targets, predicts, average=None)
                    classwise = self.resolve_nan(classwise)
                    video_log.append(classwise.reshape([1, -1]))

                # Concatenate results and calculate mean and std across videos
                video_log = np.concatenate(video_log, axis=0)
                videowise = np.nanmean(video_log, axis=0)
                ...
                # Store mean and std for the current component
